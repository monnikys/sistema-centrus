...
const GerenciarUsuarios = ({ usuarioAtual }) => {
  ...
  const cancelarEdicao = () => {
    ...
    setNovoUsuario({ nome: '', email: '', senha: '', tipo: 'usuario', permissoes: [] }) // Limpa o formulário
  ...
  const handleSubmit = async (e) => {
    ...
    if (resultado.sucesso) {
      ...
      mostrarMensagem('sucesso', modoEdicao ? 'Usuário atualizado com sucesso!' : 'Usuário criado com sucesso!') // Exibir mensagem de sucesso
      setNovoUsuario({ nome: '', email: '', senha: '', tipo: 'usuario', permissoes: [] }) // Limpar os campos do formulário
      ...
      {mostrarForm && ( // Se mostrarForm for verdadeiro
        <div className="form-usuario-card">
          ...
          <form onSubmit={handleSubmit} className="form-usuario">
            ...
            {/* Seção de Permissões */}
            <div className="permissoes-section"> {/* Seção de permissões */}
              <div className="permissoes-header"> {/* Cabeçalho das permissões */}
                <label>
                  <Shield size={18} />
                  Permissões de Acesso
                </label>
                <div className="permissoes-actions"> {/* Ações de permissões */}
                  <button
                    type="button"
              ...
              {novoUsuario.tipo === 'admin' ? (
                <div className="aviso-admin"> {/* Aviso para admin */}
                  <AlertCircle size={18} />
                  <p>
                  ...
                  <div className="permissoes-grid"> {/* Grid de permissões */}
                    ...
                    <div key={categoria} className="categoria-permissoes"> {/* Categoria de permissões */}
                      ...
                      {paginas.map((pagina) => {
                        ...
                        const isChecked = novoUsuario.permissoes.includes(pagina.id) // Verifica se a página está selecionada
            ...
            <div key={usuario.id} className="usuario-card"> {/* Cartão do usuário */}
              <div className="usuario-info"> {/* Div das informações do usuário */}
                ...
                <div className="usuario-detalhes"> {/* Div das detalhes do usuário */}
                  ...
                  {usuario.tipo !== 'admin' && usuario.permissoes && usuario.permissoes.length > 0 && (
                    <div className="usuario-permissoes"> {/* Div das permissões do usuário */}
                      <strong>Acessos:</strong>
                      <div className="permissoes-badges"> {/* Badges das permissões */}
                        {usuario.permissoes.slice(0, 3).map((permissaoId) => (
                          <span key={permissaoId} className="badge-permissao">
